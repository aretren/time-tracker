document.addEventListener('DOMContentLoaded', () => {
    ...
    const getDayAvailability = async (members, year, month, day, adminUser) => {
        ...
        // --- Get training data for all members for this day ---
        ...
        members.forEach(username => {
            ...
            memberTrainings.set(username, trainings.map(d => d.getHours())); // Store only hours for efficiency
        });
        ...
        const workingHours = Array.from({ length: 14 }, (_, i) => i + 9); // 9:00 to 22:00
        ...
        for (const hour of workingHours) {
            const memberStatusesForHour = members.map(member => {
                ...
                // --- Check for trainings ---
                const userTrainingHours = memberTrainings.get(member);
                const isTrainingHour = userTrainingHours.some(trainingStartHour => {
                    return hour === trainingStartHour || hour === trainingStartHour + 1;
                });

                if (isTrainingHour) {
                    return 'busy'; // A training makes the hour busy, overriding any other status.
                }
        ...
        return { 
            hasTwoConsecutive,
            hasSingleOverlap: sortedOverlapHours.length > 0,
            oneBusy: [...oneBusyOverlapHours], 
            oneMaybe: [...oneMaybeOverlapHours] 
        };
    };
    ...
    const renderSuggestedDays = async (date) => {
        ...
        const widgetEl = document.getElementById('suggested-days-container'); // Get the widget element
        widgetEl.classList.add('is-loading'); // Add loading class

        // Clear previous content to show it's working
        ...
        setTimeout(async () => {
            ...
            if (members.length === 0) {
                suggestedDaysList.innerHTML = '<p>Нет участников в проекте.</p>';
                return;
            }
        
            const daysInMonth = new Date(year, month + 1, 0).getDate();
            const dayElements = []; // This was accidentally removed
            let foundDays = false;
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                if (availability.hasTwoConsecutive) {
                    dayElement = document.createElement('div');
                    dayElement.className = 'day-suggestion-item highlight-green';
                } else if (availability.hasSingleOverlap) {
                    dayElement = document.createElement('div');
                    dayElement.className = 'day-suggestion-item highlight-green-yellow';
            ...
            // Clear spinner and render results
            suggestedDaysList.innerHTML = '';
            dayElements.forEach(el => suggestedDaysList.appendChild(el));
            ...
            if (!foundDays) {
                ...
                widgetEl.classList.remove('is-loading'); // Remove loading class
            }
        }, 0); // Use setTimeout to allow the UI to update and show the spinner
    };
