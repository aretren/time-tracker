document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderSuggestedDays = async (year, month) => {
        if (year === undefined || month === undefined) {
            console.error("renderSuggestedDays called with invalid date:", {year, month});
            return;
        }
        console.log('Checking for suggested days in:', {year, month});
    ...
    calendar = new VanillaCalendar('#calendar-container', {
        actions: {
            ...
            clickMonth(e, month, year) {
                // This handles user navigation through the calendar months
                renderSuggestedDays(year, month);
            },
        },
        settings: { 
            lang: 'ru', 
    ...
    projectRef.on('value', (snapshot) => {
        ...
        if (projectData) {
            ...
            // Render everything now that we have project data.
            const headerEl = projectNameHeader.parentElement;
            headerEl.innerHTML = ''; // Clear header
            ...
            headerEl.appendChild(projectInfoContainer);
            headerEl.appendChild(backToProjectsBtn);
            renderTrainings(projectData.trainings);
            renderCombinedSchedule(projectMembers, selectedDate);
            ...
            let month = calendar.settings.selected.month;
            let year = calendar.settings.selected.year;
