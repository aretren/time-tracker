document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkForIncompleteDays = async (date) => {
        ...
        if (!listEl) return;
        const widgetEl = document.getElementById('incomplete-days-widget'); // Get the widget element
        ...
        setTimeout(async () => {
            ...
            const workingHoursCount = 14; // с 9:00 до 22:00
    ...
    const applyTemplate = async (applyTo) => {
        ...
        const targetDaysStr = applyTo === 'weekdays' 
            ? 'будние дни' 
            : applyTo === 'weekends' ? 'выходные' : 'все дни';
